            ...
            <div className="bg-gradient-to-b from-sky-200 to-fuchsia-200 h-full w-60">
                ...
                </section>
                {
                    navTreeSections.map(({ title, pages }, sectionIndex) => (
                        <section key={title} className='border-b-2 border-gray-500/25 min-h-10 p-2'>
                            <>
                                {sectionIndex == 0 &&
                                    <>
                                        <h1 className='text-xl underline text-center pb-2 pt-4 pb-4'>{title.toUpperCase()}</h1>
                                        <ul className='pl-5'>
                                            {pages.map((page) => (
                                ...
                                {sectionIndex > 0 &&
                                    <>
                                        <SideBarCollapsible title={title.toUpperCase()} startOpen={pages.some(page => page.link === path)}>
                                            <ul className='pl-5'>
                                                {pages.map((page) => (
                                                    <li key={page.link} className="flex gap-2 items-center">
                                                        <ArrowRightIcon /><a href={page.link} className="hover:underline">{page.title}</a>
